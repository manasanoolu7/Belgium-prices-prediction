...
STATBEL_REAL_ESTATE_CSV_FILEPATH = os.path.dirname(
    os.getcwd()) + r"\data" + "\statbel_date_facades_price_median_and_selling_by_municipality.csv"
MUNICIPALITIES_CSV_FILEPATH = os.path.dirname(os.getcwd()) + r"\data" + "\zipcode-belgium.csv"
CLEANED_CSV_FILEPATH = os.path.dirname(os.getcwd()) + "\\assets" + "\\outputs" + "\\df_with_statbel.csv"



""" CONSIDERATIONS
Indexed rate of price over year would have been better but influence expected to be not so high 
"""
...
def add_postcode_price(statbel_csv_filepath: str = STATBEL_REAL_ESTATE_CSV_FILEPATH,
    ...
    sb_postcode = statbel_pc.groupby(by="postcode").agg(sum)
    sb_postcode["price_m_by_"] = sb_postcode["pm_per_s"] / sb_postcode["sellings"]
    sb_postcode["price_m_by_"] = sb_postcode["price_m_by_"].transform(lambda x: round(x, 2))
    ...
    statbel_pc = statbel_pc.merge(sb_postcode.loc[:, "price_m_by_"], on='postcode', how='left')
    statbel_pc.rename(columns={"price_m_by_": "price_m_by_postcode"}, inplace=True)
    ...
    out_columns = ["postcode", "price_m_by_postcode", "price_m_by_district", "price_m_by_province", "price_m_by_region"]

    real_estate_out = real_estate_out.merge(statbel_pc.loc[:, out_columns], on='postcode', how='left')

    return real_estate_out

df = add_postcode_price()
